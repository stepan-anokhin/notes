...
**Определение 1.2**: Множество $T$ будем называть *множеством индексов* последовательности $\{x\_{\alpha}\}$.

**Определение 1.3**: Мощность множества индексов $\tau = |T|$ будем называть *длиной* последовательности $x=\{x\_{\alpha}\}$, а также *количеством элементов* последовательности $x$.

**Соглашение 1**: Множества индексов мы обычно будем обозначать буквой $T$. 

Теперь определим, что такое операция от бесконечного числа аргументов. 

**Определение 2.1**: Пусть $\tau=|T|$. Тогда $\tau$-*арной* операцией над множеством $X$ будем называть любую функцию $f:X^{T}\rightarrow X$, которая отображает последовательности из $X^T$ в $X$. 

**Определение 2.2**: Множество индексов $T$ будем называть *сигнатурой* операции $f$. 

**Соглашение 2.1**: Если в некотором контексте речь будет идти о разных функциях $f, g$, то для обозначения соответствующих сигнатур мы будем использовать индексы $T_f, T_g$. Если функции индексированы $f_1, f_2, ..., f_i, ...$, то для разрешения неоднозначности мы будем использовать те же индексы у соответствующих сигнатур: $T_1, T_2, ..., T_i,...$.
...
**Определение 3.2**: Будем говорить, что множество $A$ *замкнуто относительно набора оперций* $F$, если $A$ замкнуто относительно каждой операции $f \in F$, а именно $\forall f \in F: f(A^{T\_f})\subseteq A$. Если обозначить $F(A) = \bigcup_{f \in F} f(A^{T_f})$, то замкнутость относительно набора операций можно обозначить как $F(A) \subseteq A$
