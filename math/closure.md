...
**Определение 1 (Последовательности)**: Пусть задано некоторое множество $X$ и множество $T$ произвольной 
мощности $\tau$. *Последовательностью* $\{x\_{\alpha}\}$ будем называть любую функцию $f:T\rightarrow X$, 
которая каждому $\alpha$ из $T$ ставит в соответствие некоторый элемент $x\_\alpha$ из $X$. Множество всех 
таких последовательностей будем обозначать $X^{T}$. Множество $T$ будем называть *множеством индексов* 
последовательности $\{x\_{\alpha}\}$.Мощность множества индексов $\tau = |T|$ будем называть *длиной* 
последовательности $x=\{x\_{\alpha}\}$, а также *количеством элементов* последовательности $x$. 
Множества индексов мы обычно будем обозначать буквой $T$.

Теперь определим, что такое операция от бесконечного числа аргументов. 

**Определение 2 (Операции)**: Пусть $\tau=|T|$. Тогда $\tau$-*арной* операцией над множеством 
$X$ будем называть любую функцию $f:X^{T}\rightarrow X$, которая отображает последовательности 
из $X^T$ в $X$. Множество индексов $T$ будем называть *сигнатурой* операции $f$. Если в некотором 
контексте речь будет идти о разных функциях $f, g$, то для различения соответствующих сигнатур 
мы будем писать $T\_f, T\_g$. Если функции индексированы $f\_1, f\_2, ..., f\_i, ...$, то для 
различения соответствующих сигнатур мы будем использовать те же индексы: $T\_1, T\_2, ..., T\_i, ...$. 
*Арностью* $Arity(f)$ операции $f$ мы будем называть мощность её сигнатуры $Arity(f)=|T_f|$. 

**Определение 3 (Замыкание)**: Пусть на множестве $X$ задана операция $f:X^T\rightarrow X$. 
